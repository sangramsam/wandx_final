...
@Component({
  selector: 'app-exchange-orderbook-sidepane-wan',
  templateUrl: './exchange-orderbook-sidepane-wan.component.html',
  styleUrls: ['./exchange-orderbook-sidepane-wan.component.css']
})
export class ExchangeOrderbookSidepaneWanComponent implements OnInit {
  ...
  getBuyOrders() {
    ...
    this.http.get(Constants.ServiceURLWAN + 'order/buy/getall/' + this.selectedPlatformToken.id, requestOptions).subscribe(
      data => {
  ...
  getSellOrders() {
    ...
    let requestOptions = new RequestOptions({headers: headers});
    this.http.get(Constants.ServiceURLWAN + 'order/sell/getall/' + this.selectedPlatformToken.id, requestOptions).subscribe(
      data => {
        console.log(data.json())
        var d = data.json();
  ...
  buyOrder() {
   ...
   // let web3 = this.web3Service.getWeb3();
    ...
    var buyFeeToken = Constants.EtherTokenAddressWAN;
    if (this.useWandxForBuyFee)
      buyFeeToken = Constants.WandxTokenAddressWAN;
    ...
    var data = instanceOrderTraderContract.fillOrder.getData(tokensAndAddresses, volumes, this.selectedSellOrder.ExpiringBlock, 1, ecSignature.v, ecSignature.r, ecSignature.s, sanitizedOrderId,{'from': userAccount});
    const txParams = {
    ...
    // instanceOrderTraderContract.fillOrder( //change
...
this.notificationService.showNotification(new MessageModel(MessageType.Error, 'Transaction has not been processed'), MessageContentType.Text);
  ...
  else
  {
    ...
    this.notificationService.showNotification(new MessageModel(MessageType.Success, 'Transaction has been submitted to the blockchain. Please wait for confirmation on the Ethereum blockchain'), MessageContentType.Text);
          ...
          this.selectedSellOrder = undefined;
          this.isBuyModalVisible = false;
          this.http.post(Constants.ServiceURLWAN + 'ordertransaction/create', orderTransaction, requestOptions).subscribe(
            data => {
              this.getBuyOrders();
              this.getSellOrders();
              this.isBuyModalVisible = false;
  ...
  sellOrder() {
   ...
   var orderTraderContract = this._web3.eth.contract(Constants.OrderbookContractAbi);
    ...
    var sanitizedOrderId = orderID.replace(re, '');
    sanitizedOrderId = '0x' + sanitizedOrderId;
    var sellFeeToken = Constants.EtherTokenAddressWAN;
    if (this.useWandxForSellFee)
      sellFeeToken = Constants.WandxTokenAddressWAN;
    var tokensAndAddresses = [
    ...
    console.log("Getting gas estimate");
    console.log(userAccount)
    console.log(tokensAndAddresses, volumes, this.selectedBuyOrder.ExpiringBlock,
      0, ecSignature.v, ecSignature.r, ecSignature.s, sanitizedOrderId)
    var data = instanceOrderTraderContract.fillOrder.getData(
      tokensAndAddresses, volumes, 
      this.selectedBuyOrder.ExpiringBlock,
      0, 
      ecSignature.v, 
      ecSignature.r, 
      ecSignature.s, 
    ...
    this._web3.eth.sendRawTransaction("0x"+serializedTx.toString('hex'), (err, hash)=>{
        ...
        console.log(hash);
        this.sellOrdercheck(hash,userAccount);
  		}	
		})
    // instanceOrderTraderContract.fillOrder(change
