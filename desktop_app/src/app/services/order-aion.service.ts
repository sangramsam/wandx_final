...
import { PortfolioAionService } from './portfolio-aion.service';
//import {PortfolioService} from '../services/portfolio.service';

@Injectable()
export class OrderAionService {
  ...
  constructor(private http: Http, private portfolioService: PortfolioAionService,
  ...
  getBoughtOrders() {
    if(sessionStorage.getItem('exchange')=='eth') {
      if (!this.auth.isAuthenticated() || !this.tokenService.getToken())
        return;
    ...
    if(sessionStorage.getItem('exchange')=='wan') {
      ...
      let requestOptions = new RequestOptions({headers: headers});
      this.http.get(Constants.ServiceURLWAN + 'transaction/bought', requestOptions).subscribe(
        data => {
          this._currentBoughtOrders = data.json();
          this._boughtOrders.next(this._currentBoughtOrders);
        },
  ...
  getSoldOrders() {
    ...
    if(sessionStorage.getItem('exchange')=='wan') {
      ...
      let requestOptions = new RequestOptions({headers: headers});
      this.http.get(Constants.ServiceURLWAN + 'transaction/sold', requestOptions).subscribe(
        data => {
          this._currentSoldOrders = data.json();
          this._soldOrders.next(this._currentSoldOrders);
        },
  ...
  getQuotesOrder() {
    ...
    if(sessionStorage.getItem('exchange')=='wan') {
      ...
      let requestOptions = new RequestOptions({headers: headers});
      this.http.get(Constants.ServiceURLWAN + '/portfolio/buyable', requestOptions).subscribe(
        data => {
          this._currentQuotesOrders = data.json();
          this._currentQuotesOrders.map((key,value) => {
            let assetAnalysisInput = [];
