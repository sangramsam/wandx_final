...
export class ChartService {
  ...
  setUSD(callback) {
    if (!this.auth.isAuthenticated())
      ...
      if(sessionStorage.getItem('exchange')=='aion') {
        return this.http.get('https://min-api.cryptocompare.com/data/price?fsym=AION&tsyms=USD')
        .subscribe(
          data => {
      ...
      if(sessionStorage.getItem('exchange')=='wan') {
        return this.http.get('https://min-api.cryptocompare.com/data/price?fsym=WAN&tsyms=USD')
        .subscribe(
          data => {
            console.log("got USD", data.json().USD);
            this.USD = data.json().USD;
  ...
  getUSDETHWAND(token, callback) {
    ...
    if(sessionStorage.getItem('exchange')=='eth') {
      return this.http.get('https://min-api.cryptocompare.com/data/price?fsym=' + token + '&tsyms=ETH,USD,WAND')
        ...
        return this.http.get('https://min-api.cryptocompare.com/data/price?fsym=' + token + '&tsyms=AION,USD,WAND')
        .subscribe(
          data => {
            callback(null, data.json())
          },err => {
        ...
        return this.http.get('https://min-api.cryptocompare.com/data/price?fsym=' + token + '&tsyms=WAN,USD,WAND')
        .subscribe(
          data => {
            callback(null, data.json())
          },err => {
  ...
  getTokenLowAndHighPriceOrderbook(token: any) {
    ...
    if(sessionStorage.getItem('exchange')=='aion') {
      return new Promise((resolve, reject) => {
        return this.http.get('https://min-api.cryptocompare.com/data/pricemultifull?tsyms=' + token + '&fsyms=AION')
        .subscribe(
          data => {
            if (data.json().RAW) {
              this._tokenLowAndHighPrice.next(data.json().RAW.AION[token]);
              resolve(data.json().RAW.AION[token]);
            }
            else {
              resolve(0.0);
            }
    ...
    if(sessionStorage.getItem('exchange')=='wan') {
      return new Promise((resolve, reject) => {
        return this.http.get('https://min-api.cryptocompare.com/data/pricemultifull?tsyms=' + token + '&fsyms=WAN')
          .subscribe(
            data => {
              if (data.json().RAW) {
                this._tokenLowAndHighPrice.next(data.json().RAW.WAN[token]);
                resolve(data.json().RAW.WAN[token]);
              }
              else {
                resolve(0.0);
              }
  ...
  getTokenTodayPriceOrderBook(token: any) {
    ...
    if(sessionStorage.getItem('exchange')=='wan') {
      return new Promise((resolve, reject) => {
        return this.http.get('https://min-api.cryptocompare.com/data/pricehistorical?fsym=' + token + '&tsyms=WAN&ts=' + new Date().getTime() + '&extraParams=WANDX')
          .subscribe(
            data => {
              this.temp = data.json();
              console.log("temp", this.temp[token]);
              if (this.temp[token]) {
                // this._tokenTodayPrice.next(this.temp[this.selectedToken].ETH.toFixed(4));
                resolve(this.temp[token].WAN.toFixed(4))
              }
              else {
                //this._tokenTodayPrice.next((0.0).toFixed(4));
                resolve((0.0).toFixed(4))
