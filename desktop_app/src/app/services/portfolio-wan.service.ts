...
import { WalletWanService } from './wallet-wan.service';
...
@Injectable()
export class PortfolioWanService {
  ccadd: any
  ...
  publishPortfolio(portfolioName: string, askingPriceInWand: number, creationPriceInWand: number, portfolio: Array<any>) {
      ...
      publishRequestObject['UserAccount'] = sessionStorage.getItem("walletAddress");
      ...
      this.http.post(Constants.ServiceURLWAN + '/portfolio/create', publishRequestObject, requestOptions).subscribe(
        data => {
  ...
  signAndPublishPortfolio(portfolioName: string, askingPriceInWand: number, creationPriceInWand: number,
    ...
    if (!this.auth.isAuthenticated())
      return;
    var publishRequestObject = {};
    var maker = this.WanWeb3Service._getWeb3().eth.coinbase;
    var orderID = UUID.UUID();
    var web3Instance = this.WanWeb3Service._getWeb3();
  ...
  private validateAssetsForBuyer(portfolio: SellablePortfolio, quote: Quote) {
      ...
      let requestOptions = new RequestOptions({ headers: headers });
      let data = ['WXETH'];
      this.http.post(Constants.ServiceURLWAN + 'manage/token/summary/' + portfolio.PortfolioId + '/' + quote.UserAccount, data, requestOptions).subscribe(
        data => {
  ...
  private processBuy(portfolio) {
    ...
    // }
      ...
      if (sessionStorage.getItem("walletAddress") === null || sessionStorage.getItem("walletAddress") === undefined || sessionStorage.getItem("walletAddress").length === 0) {
        this.transactionInProgress = false;
        this.notificationService.showNotification(new MessageModel(MessageType.Error, 'Failed to get Wallet Instance'), MessageContentType.Text);
        return;
      }
      ...
      var data = vsbContract.buy.getData({ from: sessionStorage.getItem("walletAddress"), value: portfolio.valueInEther * 1000000000000000000 });
      const txParams = {
  ...
  private processSell(portfolio: SellablePortfolio, quote: Quote) {
    ...
    var seller = web3Instance.eth.coinbase;
    let dexContractValue = this.getContract('DEX');
    let wxEthContractValue = this.getContract('WXETH');
    if (dexContractValue === undefined) {
      this.transactionInProgress = false;
  ...
  getList() {
    ...
    if (!this.tokenService.getToken()) {
     ...
     // let web3Instance = this.web3.getWeb3();
      ...
      let query = { currentOwnerOrSeller: sessionStorage.getItem('walletAddress') };
      let query1 = { owner: sessionStorage.getItem('walletAddress') };
      body.set('query', JSON.stringify(query));
      body1.set('query', JSON.stringify(query1));
      //console.log('form data', body.toString());
      let requestOptions = new RequestOptions({ headers: headers });
      this.http.post(Constants.BashketURLWAN, body.toString(), requestOptions).subscribe(data => {
      ...
      this.http.post(Constants.BashketURLWAN, body1.toString(), requestOptions).subscribe(data1 => {
  ...
  requestPortfolio(theme) {
    return new Promise((resolve, reject) => {
      ...
      publishRequestObject['UserAccount'] = this.WanWeb3Service._getWeb3().eth.coinbase;
